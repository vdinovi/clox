...
Token scanner_scan(Scanner *scanner) {
    ...
    for (bool skip = true; skip;) {
        start = peek(scanner);
        if (eof(scanner)) return TOKEN(TOKEN_EOF, NULL, 0, scanner->line);
        switch (*start) {
        case '\n': scanner->line++;
        case ' ': 
        case '\t':
    ...
    switch (*start) {
    case '(': return TOKEN(TOKEN_LEFT_PAREN, advance(scanner), 1, scanner->line);
    case ')': return TOKEN(TOKEN_RIGHT_PAREN, advance(scanner), 1, scanner->line);
    case '{': return TOKEN(TOKEN_LEFT_BRACE, advance(scanner), 1, scanner->line);
    case '}': return TOKEN(TOKEN_RIGHT_BRACE, advance(scanner), 1, scanner->line);
    case ',': return TOKEN(TOKEN_COMMA, advance(scanner), 1, scanner->line);
    case '.': return TOKEN(TOKEN_DOT, advance(scanner), 1, scanner->line);
    case '+': return TOKEN(TOKEN_PLUS, advance(scanner), 1, scanner->line);
    case '-': return TOKEN(TOKEN_MINUS, advance(scanner), 1, scanner->line);
    case ';': return TOKEN(TOKEN_SEMICOLON, advance(scanner), 1, scanner->line);
    case '*': return TOKEN(TOKEN_STAR, advance(scanner), 1, scanner->line);
    case '!':
        advance(scanner);
        return match(scanner, '=') ?
            TOKEN(TOKEN_BANG_EQUAL, start, 2, scanner->line) :
            TOKEN(TOKEN_BANG, start, 1, scanner->line);
    case '=':
        advance(scanner);
        return match(scanner, '=') ?
            TOKEN(TOKEN_EQUAL_EQUAL, start, 2, scanner->line) :
            TOKEN(TOKEN_EQUAL, start, 1, scanner->line);
    case '>':
        advance(scanner);
        return match(scanner, '=') ?
            TOKEN(TOKEN_GREATER_EQUAL, start, 2, scanner->line) :
            TOKEN(TOKEN_GREATER, start, 1, scanner->line);
    case '<':
        advance(scanner);
        return match(scanner, '=') ?
            TOKEN(TOKEN_LESS_EQUAL, start, 2, scanner->line) :
    ...
    if (is_alpha(*start)) return scan_identifier(scanner);
    if (is_digit(*start)) return scan_number(scanner);
...
String* token_repr(Token *token, Allocator *alloc) {
    return string_sprintf(
        alloc, "Token { type=%s, start=\"%.*s\", length=%d, line=%d }",
        token_type_name(token->type), token->length, token->start, token->length, token->line
    );
}
...
static inline bool match(Scanner *scanner, const char ch) {
    if (eof(scanner)) return false;
    if (*peek(scanner) != ch) return false;
    advance(scanner);
    return true;
} 
...
static Token scan_string(Scanner *scanner) {
    ...
    if (eof(scanner)) return scan_error(scanner, line, "Unterminated string");
...
static Token scan_number(Scanner *scanner) {
    ...
    const char *ch = peek(scanner);
    for (; !eof(scanner) && is_digit(*ch); ch = peek(scanner)) advance(scanner);
